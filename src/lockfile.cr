...
class Lockfile
  ...
  class SleepCycle < Array
    ...
    def initialize(@min : Float32, max : Float32, inc : Float32)
      @range = @max - @min
  ...
  def initialize(path, opts = {} of String => String | Int32 | Float32?, &block)
    @klass = self.class
    @path = path
    @opts = opts
  ...
  def lock
    ...
    attempt do
      begin
            ...
            Timeout.timeout(@timeout) do
              ...
              begin
                ...
                begin
                  ...
                  raise StatLockError, "stat's do not agree" unless tmp_stat.rdev == lock_stat.rdev && tmp_stat.ino == lock_stat.ino
                ...
                case validlock?
                when true
                  raise MaxTriesLockError, "surpased retries <#{@retries}>" if @retries && n_retries >= @retries
        ...
        if block_given?
          ...
          begin
            ...
            begin
              @refresher.kill if @refresher && @refresher.status
              @refresher = nil
            ensure
      ...
      rescue e : Errno::ESTALE | Errno::EIO
  ...
  def create(path)
    ...
    begin
      ...
      f = open path, File::WRONLY | File::CREAT | File::EXCL, 0o0644
    ensure
      File.umask umask if umask
    end
    return(block_given? ? begin
      yield f
    ensure
      f.close
    end : f)
